...
<div class="flex flex-col md:flex-row md:items-center inputblock">    
    ...
    @elseif($type == 'checkbox')
        <div>
            <div class="my-2 dark:text-white">{{ $label }} </div>
            <ul class="grid w-full gap-6 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4">
                @foreach ($options as $option_name => $option_value)
                    <li>
                        <input type="checkbox" id="permission_{{ $option_name }}" name="{{ $option_value }}"
                            class="hidden peer" value="yes" />
    
                        <label for="permission_{{ $option_name }}"
